...
const outputDir = path.join(rootDir, "fixtures", "full-review");
fs.mkdirSync(outputDir, { recursive: true });

async function main() {
  console.log("--- Iniciando Suite Integral de Verificación (Escenario, Volumen y Viewports) ---");
  ...
  const viteServer = await createViteServer({
    ...
    plugins: [
      {
        name: "full-review-api",
        configureServer(server) {
          server.middlewares.use("/api/upload", (req, res) => {
            if (req.method === "POST") {
              const url = new URL(req.url, `http://localhost:${vitePort}`);
              const fileName = url.searchParams.get("name") || "unknown.bin";
              const targetPath = path.join(outputDir, fileName);
              const fileStream = fs.createWriteStream(targetPath);
              req.pipe(fileStream);
              req.on("end", () => {
                fileStream.close();
                console.log(`[Servidor] Artefacto guardado: ${fileName}`);
  ...
  const targetUrl = `http://localhost:${vitePort}/scripts/fullVerificationRunner.html`;
  ...
  const timeout = setTimeout(() => {
    console.error("TIMEOUT: La verificación integral tardó más de 60 segundos.");
  ...
  console.log("\n=======================================================");
  console.log("REPORTE DE VERIFICACIÓN INTEGRAL");
  console.log("=======================================================");
  console.log(JSON.stringify(report, null, 2));

  if (report.error) {
    console.error("Error reportado durante la suite:", report.error);
    process.exit(1);
  }

  console.log("\n--- Inspección de Artefactos Generados en fixtures/full-review ---");
  ...
  console.log("\nSuite integral completada exitosamente.");
}
