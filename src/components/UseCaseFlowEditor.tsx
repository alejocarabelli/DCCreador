...
import { buildProjectSymbolIndex, type ProjectSymbolClass, type ProjectSymbolIndex } from '../utils/projectSymbolIndex';
...
type StepField = keyof Omit<UseCaseFlowStep, 'id'>;
type FlowTextField = Extract<StepField, 'actor' | 'system'>;
...
const unique = <Value extends string>(values: Value[]): Value[] => Array.from(new Set(values));

const matchesPrefix = (value: string, query: string): boolean =>
...
const buildFlowUsageIndex = (content: UseCaseFlowContent): FlowUsageIndex => {
...
const getMarkerLength = (line: string): number => {
  const numberedMatch = numberedLinePattern.exec(line);
...
const setCaretPositionAfterRender = (element: HTMLTextAreaElement | HTMLInputElement, position: number): void => {
  window.requestAnimationFrame(() => {
    element.focus();
...
const setStateBulletCaretAfterRender = (
  element: HTMLTextAreaElement,
  lineIndex: number,
  markerLength: number,
): void => {
...
const normalizeStateBulletShortcut = (
  value: string,
  position: number,
...
const removeOrPromoteStateBullet = (
  value: string,
  position: number,
): { lineIndex: number; markerLength: number; value: string } | null => {
...
const removeOrPromoteFlowMarker = (
  value: string,
  position: number,
): { lineIndex: number; markerLength: number; value: string } | null => {
  const { lineColumn, lineIndex, lines } = getLineInfo(value, position);
...
const ensureStepMarker = (value: string): string => {
...
function AutoGrowTextarea({ inputRef, minRows = 1, onInput, value, ...props }: AutoGrowTextareaProps) {
